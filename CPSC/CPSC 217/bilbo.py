...
Highscores are saved in a separate file, "scores.txt" in the form "Gold,HP" according to the rule outlined in the assignment.
...
#This fuction will create and store a list of information regarding Bilbo's health, gold, and his invulnerability state.
...
#Entering 'i' turns on invulnerabilty, 'd' turns on debugging mode.
...
#The board displayer below is similar to the one given to us by James Tam.
    for r in range (0,R_SIZE,1):
        for c in range (0,C_SIZE,1):
            sys.stdout.write(str(erebor[r][c]))
        print()
    print ("""
Movement options
----------------
...
def hobbitMove(moveSelect, erebor, gameState, bilboStats, debug, invuln, gameWin, mode2):
    validMove = True
#These following branches, will group like numbers together based on the typical comput number pad and their result. the first 3 branches describe the vertical movement, while the latter 3 the horizontal movement. Also a compass direction is assigned to each movement, to be used for debugging purposes.
...
#These two lines above reset Bilbos position so that when the user again is promted to move him, he will be at his new position and any changes will occur from there.
...
def intro():
#The following excerpt is courtesy of, "The Hobbit" by J.R.R. Tolkien.
    print ("""
...
#Main function containg all other functions.
def main():
